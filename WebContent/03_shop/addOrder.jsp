...
<body>
	<%
		...
		System.out.println(id);		// 콘솔 실행했는데 null로 출력됨 
	
		// 아이디가 없으면? 상품 id가 넘어오지 않았을 때 강제로 products.jsp 이동시키고 종료 
		if (id != null || id.trim().equals("")) {
			// products.jsp로 되돌아간다. 
			response.sendRedirect("shoppingInfo_order.jsp");
			// 프로그램 종료 
		} 
		...
		ArrayList<Product> list = (ArrayList<Product>) session.getAttribute("onlyOrderList");
		...
		session.setAttribute("onlyOrderList", list);
		session.getAttribute("onlyOrderList"); 
	%>
</body>
</html>